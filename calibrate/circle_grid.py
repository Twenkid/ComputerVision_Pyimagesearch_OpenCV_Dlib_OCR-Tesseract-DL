...
# Updated to OpenCV 4.5.3: OK
# Added proper YAML loading example at the end, see also y.py
# Circle pattern: https://nerian.com/support/calibration-patterns/
...
while(found < num):  # Here, 10 can be changed to whatever number you like to choose
    ...
    ret, corners = cv2.findCirclesGrid(im_with_keypoints, (v1,v2), None, flags = cv2.CALIB_CB_ASYMMETRIC_GRID)   # Find the circle grid
...
with open(calibrationFile, 'r') as stream:
    dictionary = yaml.safe_load(stream)
camera_matrix_r = dictionary.get("camera_matrix")
dist_coeffs_r = dictionary.get("dist_coeff")
